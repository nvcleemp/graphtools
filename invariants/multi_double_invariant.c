...
double INVARIANT(GRAPH graph, ADJACENCY adj);

int graphCount = 0;
int graphsFiltered = 0;

double epsilon =  1.0/1048576;
...
int main(int argc, char** argv) {
    ...
    while (readMultiCode(code, &length, stdin)) {
        ...
        double value = INVARIANT(graph, adj);
        if(doFiltering){
            if(allowEqual && doubleEqual(value, filterValue)){
                graphsFiltered++;
            ...
            fprintf(stdout, "Graph %d has " XSTR(INVARIANTNAME) " equal to %f.\n", graphCount, value);
        }
    }
    ...
    if(doFiltering){
        ...
        fprintf(stderr, "Graph %d has maximum value for " XSTR(INVARIANTNAME) ": %f.\n", extremumGraph, maximum);
    } else if(findMinimum){
        fprintf(stderr, "Graph %d has minimum value for " XSTR(INVARIANTNAME) ": %f.\n", extremumGraph, minimum);
    }
