 ...
 * subdivides each face with one vertex.   
 * 
 * 
 * Compile with:
 *     
 *     cc -o subdivide_vertex -O4 subdivide_vertex.c
 * 
 */
...
void writeSubdividedPlanarCodeChar(){
    int i;
    EDGE *e, *elast;
    
    //write the number of vertices of the subdivision
    fputc(nv + nf, stdout);
...
int main(int argc, char *argv[]) {
    ...
    while (readPlanarCode(code, &length, stdin)) {
        ...
        writeSubdividedPlanarCode();
        numberOfGraphs++;
    }
    
    fprintf(stderr, "Output subdivision of %d graph%s.\n", numberOfGraphs, 
                numberOfGraphs==1 ? "" : "s");
}
