 ...
 * writes the ones that are regular to standard out.   
 * 
 * 
 * Compile with:
 *     
 *     cc -o regular_pl -O4 regular_pl.c
 * 
 */
...
boolean isFaceRegular(){
    int i;
    int s = faceSize[0];
    
    for(i=1; i<nf; i++){
        if(s!=faceSize[i]) return FALSE;
    }
    return TRUE;
}

boolean isVertexRegular(){
    int i;
    int d = degree[0];
    
    for(i=1; i<nv; i++){
        if(d!=degree[i]) return FALSE;
    }
    return TRUE;
}
...
boolean isRegular(){
    if(checkFaces){
        return isFaceRegular();
    } else {
        return isVertexRegular();
    }
}
