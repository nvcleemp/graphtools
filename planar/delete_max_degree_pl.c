 ...
 * a single vertex with maximum degree, and writes the resulting graph to standard out.   
 * 
 * 
 * Compile with:
 *     
 *     cc -o delete_max_degree_pl -O4 delete_max_degree_pl.c
 * 
 */
...
void writeVertexDeletedPlanarCodeChar(){
    int i;
    EDGE *e, *elast;
...
void writeVertexDeletedPlanarCode(){
    ...
    if (newNv + 1 <= 255) {
        writeVertexDeletedPlanarCodeChar();
    } else if (newNv + 1 <= 65535) {
        writeVertexDeletedPlanarCodeShort();
    } else {
...
void help(char *name) {
    fprintf(stderr, "The program %s removes a vertex with maximum degree from a planar graph.\n", name);
    fprintf(stderr, "The vertex with maximum degree and smallest index is removed.\n\n");
...
int main(int argc, char *argv[]) {
    ...
    while (readPlanarCode(code, &length, stdin)) {
        ...
        int max_degree = 0, max_degree_vertex;
        for(i = 0; i < nv; i++){
        ...
        deleted[max_degree_vertex] = TRUE;
