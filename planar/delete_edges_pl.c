 ...
 * the specified edges, and writes the resulting graph to standard out.   
 * 
 * 
 * Compile with:
 *     
 *     cc -o delete_edges_pl -O4 delete_edges_pl.c
 * 
 */
...
void writeEdgeDeletedPlanarCodeChar(){
    int i;
    EDGE *e, *elast;
...
void writeEdgeDeletedPlanarCodeShort(){
    int i;
    EDGE *e, *elast;
...
void writeEdgeDeletedPlanarCode(){
    ...
    if (nv + 1 <= 255) {
        writeEdgeDeletedPlanarCodeChar();
    } else if (nv + 1 <= 65535) {
        writeEdgeDeletedPlanarCodeShort();
    } else {
...
void help(char *name) {
    fprintf(stderr, "The program %s removes edges from a planar graph.\n\n", name);
...
void usage(char *name) {
    fprintf(stderr, "Usage: %s [options] u1,v1 u2,v2 ...\n", name);
...
int main(int argc, char *argv[]) {
    ...
    if (readPlanarCode(code, &length, stdin)) {
        ...
        for (i = 0; i < argc - optind; i++){
            if(sscanf(argv[optind+i], "%d,%d", &from, &to) != 2){
                fprintf(stderr, "Error while reading edge to be deleted -- exiting!\n");
                fprintf(stderr, "Error happened while parsing '%s'.\n", argv[optind+i]);
                usage(name);
                return EXIT_FAILURE;
            }
            EDGE *e = findEdge(from-1, to-1);
            if(e!=NULL){
                MARK(e);
                MARK(e->inverse);
                edgesRemoved++;
        ...
        writeEdgeDeletedPlanarCode();
        fprintf(stderr, "Removed %d edge%s.\n", edgesRemoved, edgesRemoved == 1 ? "" : "s");
    } else {
